...
public class CameraZoom
{
    ...
    public void Init(Camera _cam, ref PlayerControls playerCon)
    {
        m_playerControls = playerCon;
        m_zooming = GetInputZoom();
        m_cam = _cam;
    ...
    public void ChangeFOV(MonoBehaviour _mono)
    {
        if (m_running)
        {
            ...
            m_zooming = !m_zooming;//m_camInputData.IsZooming;
            return;
        }
    ...
    IEnumerator ChangeFOVRoutine()
    {
        ...
        float _targetFOV = !GetInputZoom() ? m_initFOV : zoomFOV;
    ...
    protected bool GetInputZoom() {
        return m_playerControls.Walking.Zoom.ReadValue<float>() == 1.0f;
    }
    #endregion
}

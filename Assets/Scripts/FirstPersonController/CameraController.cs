...
public class CameraController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        InitComponents();
        ChangeCursorState();
        playerControls.Walking.Zoom.performed += ctx => HandleZoom();
    }
    ...
    void InitComponents()
    {
        ...
        cameraZoom.Init(m_cam, ref playerControls);
        cameraSway.Init(m_cam.transform);
    }

    void CalculateRotation()
    {
        m_desiredYaw += GetInputLookVector().x * sensitivity.x * Time.deltaTime;
        m_desiredPitch -= GetInputLookVector().y * sensitivity.y * Time.deltaTime;
    ...
    protected Vector2 GetInputLookVector()
    {
        return playerControls.Walking.Look.ReadValue<Vector2>();
    }

    protected bool GetInputZoom()
    {
        return playerControls.Walking.Zoom.ReadValue<float>() == 1;
    }
    #endregion
}
